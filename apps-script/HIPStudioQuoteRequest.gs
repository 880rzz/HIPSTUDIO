...
const HEADERS = [
  'request_id','received_at','response_due_at','status','owner',
...
function doGet() {
  return json_({ok:true, service:'HIPStudio quote request', version:HIPSTUDIO.PAYLOAD_VERSION});
}

function doPost(e) {
  try {
    const raw = String(e && e.postData && e.postData.contents || '');
    if (!raw || raw.length > HIPSTUDIO.MAX_PAYLOAD_BYTES) {
      return json_({ok:false,error:'invalid_payload'});
    }
    const incoming = JSON.parse(raw);

    // Honeypot: acknowledge silently so bots get no useful signal.
...
function setup() {
  ...
  return {sheetId:id, sheetUrl:ss.getUrl(), senderAliasAvailable:senderOptions_().aliasAvailable};
}
...
function sendInternal_(r) {
  const subject = '[HIPStudio ajánlatkérés ' + r.request_id + '] ' + (r.company || r.name) + ' — ' + compact_(r.pillars,80);
  const html = '<div style="font-family:Arial,sans-serif;line-height:1.5;color:#111">' +
    '<h1>Új HIPStudio ajánlatkérés</h1>' +
    '<p><strong>Azonosító:</strong> ' + h_(r.request_id) + '<br>' +
    '<strong>24 órás válasz-határidő:</strong> ' + h_(formatDate_(r.response_due_at)) + '</p>' +
    section_('Kapcsolat', [
      ['Név',r.name],['Cég',r.company],['E-mail',r.email],['Telefon',r.phone],['Preferált kapcsolat',r.preferred_contact]
    ]) +
    section_('Projekt', [
      ['Terület',r.pillars],['Szolgáltatás',r.services],['Cél',r.goals],['Kiinduló helyzet',r.project_summary],['Sikeres eredmény',r.desired_outcome],['Must-have',r.must_have]
    ]) +
    section_('Időzítés és helyszín', [
      ['Határidő',r.deadline],['Időpontok',[r.preferred_date_1,r.preferred_date_2,r.preferred_date_3].filter(Boolean).join(', ')],['Rugalmasság',r.date_flexibility],['Helyszíntípus',r.location_type],['Helyszín',r.location_details],['Referencia / brief',r.reference_url]
    ]) +
    ...
    '<hr><p>Válaszolj az ügyfélnek az <a href="mailto:' + h_(HIPSTUDIO.CENTRAL_EMAIL) + '">' + h_(HIPSTUDIO.CENTRAL_EMAIL) + '</a> központi cím használatával.</p></div>';
