...
const representative=['/hu/','/en/','/de/','/hu/uzleti-mukodes/','/hu/kreativ-tartalom/','/hu/vallalati-elmenyek/','/hu/megoldasok/','/hu/megoldasok/business-operations-360/','/hu/megoldasok/content-engine/','/hu/ai-trust/','/hu/kapcsolat/'];
...
const summary={time:new Date().toISOString(),browser:await browser.version(),tested:results.length,results,errors,externalRequests:[...external],cookies,storage,interactions:{skipText,skipTarget,languageSwitch}};
...
await browser.close();
if(results.some(r=>r.status!==200||r.overflow||r.brokenImages||r.h1!==1||r.review!==1||r.violations.length)||errors.length||external.size||cookies.length||storage.localStorage||storage.sessionStorage||skipTarget!=='main'||!languageSwitch)process.exitCode=1;
