...
def render(key,l,title,desc,body,faq=None,image_id=None,page_type='WebPage',script=False):
 ...
 graph=[{'@type':['Organization','ProfessionalService'],'@id':org,'name':'HIPStudio','legalName':'Hipstudió Korlátolt Felelősségű Társaság','url':BASE+'/','sameAs':['https://www.wikidata.org/wiki/Q138482177'],'telephone':'+36302215506','email':'info@hipstudio.hu','address':{'@type':'PostalAddress','streetAddress':'Lágymányosi utca 15.','postalCode':'1111','addressLocality':'Budapest','addressCountry':'HU'},'contactPoint':{'@id':BASE+'/#contact'},'founder':{'@id':BASE+'/#norbert-banhalmi'}},
...
for l in LANGS:
 home=f'<section class="home-hero"><div class="hero-copy"><p class="eyebrow">Budapest · {e(ui("photography",l))} / Film / Podcast</p><h1>{e(ui("homeTitle",l))}</h1><p class="lead">{e(ui("homeIntro",l))}</p><a class="button" href="{href("services",l)}">{e(ui("services",l))} <span aria-hidden="true">↗</span></a></div><figure class="hero-image">{img("portrait-20",l,True)}<figcaption>HIPStudio · {e(ui("photography",l))}</figcaption></figure><div class="hero-index" aria-hidden="true">01 — HIPStudio</div></section>'
 home+=f'<section class="editorial"><p class="eyebrow">{e(ui("visualTrust",l))}</p><h2>{e(ui("homeEditorial",l))}</h2><p>{e(ui("homeBody",l))}</p></section><section class="section"><div class="section-heading"><h2>{e(ui("services",l))}</h2><a href="{href("services",l)}">{e(ui("all",l))} ↗</a></div>{cards(["business","advertising","event","video","podcast","property"],l)}</section>'
 ...
 for cat,cluster in [('portrait','portrait'),('portfolio','portrait'),('commercial','commercial'),('property','space'),('art','art')]:
  ...
  for m in IMAGES:
   if m['category']==cat:content+=f'<figure><a href="{asset("photos/"+m["id"]+"-1440.webp")}">{img(m["id"],l,hero=m["id"]=="portrait-01",gallery=True)}</a><figcaption>{e(m["alt"][l])}</figcaption></figure>'
  content+='</div></section>'
 ...
 content=heading(ui('studio',l),studio_intro,l)+f'<section class="reading"><h2>1111 Budapest, Lágymányosi utca 15.</h2><p>{e(FAQ[5][1][l])}</p><p>{e(tr("Személyes egyeztetéshez kérj időpontot emailben vagy telefonon. A helyszín pontos felszereltségét és az akadálymentes bejutás lehetőségét érkezés előtt egyeztessük. || Arrange a visit by email or telephone. Please discuss equipment and step-free access before arriving. || Vereinbare einen Besuch per E-Mail oder Telefon. Bitte kläre Ausstattung und barrierefreien Zugang vor deiner Ankunft.")[l])}</p><a class="button" href="{href("contact",l)}">{e(ui("contact",l))} ↗</a></section>'
 render('studio',l,ui('studio',l),studio_intro,content)
 partner_intro=tr('Két fő fotós partner, több mint húsz fotóssal dolgozó szakmai hálózat. || Two principal photography partners and a network working with more than twenty photographers. || Zwei zentrale Fotopartner und ein Netzwerk mit mehr als zwanzig Fotografen.')[l]
 partner_note=tr('Bánhalmi Norbert és Speier Vikó a HIPStudio két fő fotós partnere. Önálló szakmai entitások; a partnerség nem jelent automatikusan tulajdonosi vagy alkalmazotti jogviszonyt. || Bánhalmi Norbert and Speier Vikó are HIPStudio’s two principal photography partners. They are independent professional entities; partnership does not automatically imply ownership or employment. || Bánhalmi Norbert und Speier Vikó sind die beiden zentralen Fotopartner von HIPStudio. Sie sind eigenständige berufliche Akteure; Partnerschaft bedeutet nicht automatisch Eigentum oder Anstellung.')[l]
 content=heading(ui('partners',l),partner_intro,l)+f'<section class="reading"><p>{e(partner_note)}</p><h2>Bánhalmi Norbert</h2><a href="https://www.norbertbanhalmi.com/">BANHALMI ↗</a><h2>Speier Vikó</h2><a href="https://www.vikospeier.com/">Viko Speier ↗</a><p>{e(tr("A feladathoz illeszkedő csapatot az egyeztetés során állítjuk össze. További partnerek és ügyfelek neveit nem közöljük külön hozzájárulás nélkül. || The team is planned around the assignment. We do not list additional partners or clients without separate consent. || Das Team wird passend zur Aufgabe geplant. Weitere Partner oder Kunden werden ohne gesonderte Zustimmung nicht genannt.")[l])}</p></section>'
...
(D/'llms.txt').write_text('# HIPStudio\n\nPhotography, film and podcast in Budapest.\n\n'+('\n'.join('- '+p['title']+': '+p['url'] for p in PAGES))+'\n\nPricing: '+BASE+'/pricing.json\nNo inferred ratings, client lists or independent studio rental terms.\n')
(R/'audit/build.json').write_text(json.dumps({'mode':MODE,'base':BASE,'pages':PAGES,'redirects':len(mapping),'images':len(IMAGES),'css':CSS},ensure_ascii=False,indent=2))
