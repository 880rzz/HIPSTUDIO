...
for lang in LANGS:
    ...
    home = f'''<section class="hero"><div><p class="eyebrow">HIPStudio · 2006</p><h1>{e(DATA['hero']['title'][lang])}</h1><p class="lead">{emphasized(intro,lang)}</p></div></section><section class="section home-introduction"><p class="eyebrow">HIPStudio</p><h2>{emphasized(intro,lang)}</h2></section><section class="section home-three-doors"><div class="section-head"><h2>{e(UI['pillars'][lang])}</h2></div><div class="pillars">{cards}</div></section>{cta(lang)}'''
