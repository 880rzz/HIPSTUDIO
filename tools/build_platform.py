...
def pillar_page(p, lang):
    ...
    highlights = '<section class="section product-highlights"><p class="eyebrow">'+e({'hu':'Kiemelések','en':'Highlights','de':'Highlights'}[lang])+'</p><div class="highlight-track" tabindex="0" role="region" aria-label="'+e({'hu':'Szolgáltatási kiemelések','en':'Service highlights','de':'Leistungshighlights'}[lang])+'">'+''.join(f'<article><span class="highlight-index">0{i+1}</span><h3>{e(t)}</h3><p>{e(b)}</p></article>' for i,(t,b) in enumerate(zip(highlight_titles,highlight_body)))+'</div></section>'
