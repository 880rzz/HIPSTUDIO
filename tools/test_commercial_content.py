...
for lang in ('hu','en','de'):
    home = (D / ROUTES['home'][lang]).read_text()
    assert home.count('data-commercial-layer="problem-led-home"') == 1
    assert home.count('data-commercial-layer="partner-benefits"') == 1
    ...
    about = (D / ROUTES['about'][lang]).read_text()
    assert about.count('data-commercial-layer="integration-story"') == 1
    ...
    for pillar in ('business','creative','experiences'):
        page = (D / ROUTES[pillar][lang]).read_text()
        assert page.count('data-commercial-layer="pillar-value"') == 1
        assert DATA['pillars'][pillar]['problem'][lang] in page
        assert DATA['pillars'][pillar]['outcome'][lang] in page

business_hu = (D / ROUTES['business']['hu']).read_text()
assert 'előre jóváhagyott felelősségi körökön belül' in business_hu
about_hu = (D / ROUTES['about']['hu']).read_text()
