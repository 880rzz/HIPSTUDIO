...
def assert_review():
    ...
    expected_solution_keys={
        'business-operations-360','finance-control','ai-readiness',
        'content-engine','corporate-experience-design'
    }
...
def assert_quote_endpoint_gate():
    e={
      'BUILD_MODE':'production','PLATFORM_URL':'https://example.com',
      'PLATFORM_MASTER_BRAND_APPROVED':'1','PLATFORM_PUBLICATION_APPROVED':'1'
    }
...
def assert_isolated_production_contract():
    build({
      'BUILD_MODE':'production','PLATFORM_URL':'https://example.com',
      'PLATFORM_MASTER_BRAND_APPROVED':'1','PLATFORM_PUBLICATION_APPROVED':'1',
      'QUOTE_FORM_ENDPOINT':'https://script.google.com/macros/s/test-review-endpoint/exec'
    })
    manifest=json.loads((D/'platform-build.json').read_text())
    ...
    assert sitemap.count('<url><loc>')==42
    for page in manifest['pages']:
        html=(D/page['path'].strip('/')/'index.html').read_text()
